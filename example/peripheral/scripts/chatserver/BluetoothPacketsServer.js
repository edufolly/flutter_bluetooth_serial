...
class BluetoothPacketsServer {
    constructor() {
        ...
        this._closedHandler = undefined;

        this._serialPortServer.on('data', (buffer) => {
            self._chunks.push(buffer);

            if (!(self._packetHandler instanceof Function)) { // @WARN if packet handler null, packets handling is just halted!
                return null;
            }
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    if (diff >= 0) {
                        ...
                        self._packetHandler(packetType, bytes.take(packetDataLength));
        ...
        this._serialPortServer.on('closed', () => {
            if (self._closedHandler instanceof Function) {
                self._closedHandler();
            }
        });
    }
...
module.exports = BluetoothPacketsServer;
